...
public class BinaryTree {
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
        // base cases, since method is void no need to return anything
        if (node == null)
            return;

        // check if node.data is the value we want to update
        if (node.data == oldVal) {
            node.data = newVal;
            // exit out of current recursion branch
            return;
        }
        ...
        return; // return call here since all nodes have now been visited
    }
    ...
    private int findMinHelper(Node node) {
        ...
        int left = findMinHelper(node.left); // returning the lowest once the end of the data and the left, right calls
    ...
    private int nodesGTHelper(Node node, int val) {
        ...
        if (node.data > val) {
            count += 1 + nodesGTHelper(node.left, val) + nodesGTHelper(node.right, val);
        } else {
            count += nodesGTHelper(node.left, val) + nodesGTHelper(node.right, val);
        }
    ...
    private int[] averageHelper(Node n) {
        // return the array [0,0] whenever we hit a base case
        if (n == null) {
            return new int[] { 0, 0 };
        }
        ...
        int addedSum = n.data + left[0] + right[0]; // add the data of the node to left[0] and right[0] to get the total
                                                    // sum up to this point
        // when n is null we return the array [0,0] but after that we we start added
        // n.data bottom up

        int nodeCount = 1 + left[1] + right[1]; // take the same approach with the node count, but we can just add + 1
                                                // to the node to do this instead of data

        // return the addedSum and node count at this point
        return new int[] { addedSum, nodeCount };
    }
}
